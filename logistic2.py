...
def load_data():
    ...
    train_set_x=a[0:220]
    valid_set_x=a[160:240]
    test_set_x=a[150:230]
    train_set_y=y[1:221]
    valid_set_y=y[161:241]
    test_set_y=y[151:231]
    '''
    train_set_x=a[0:220]
...
def sgd_optimization_mnist(learning_rate=0.13, n_epochs=1,
    ...
    pred_data=pred_data.reshape(1,10).tolist()[0]
    profit=[]

    for i in range(len(pred_data)):
        if(pred_data[i]==1):     #做多
            profit.append(float(data1[151+i][3])-float(data1[151+i][0]))
        if(pred_data[i]==0):     #做空
            profit.append(float(data1[151+i][0])-float(data1[151+i][3]))
    income=[]
    print profit
    num=0.0

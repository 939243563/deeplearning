...
from logistic2 import LogisticRegression, load_data
...
def test_SdA(finetune_lr=0.1, pretraining_epochs=5,
    ...
    for i in range(len(pred_data)):
        if(pred_data[i]==1):     #做多
            ...
            profit.append(float(data[151+i][3])-float(data[151+i][0]))
        if(pred_data[i]==0):     #做空
            #profit.append(data[272*(11+i)][0]-data[272*(11+i+1)-2][3])
            profit.append(float(data[151+i][0])-float(data[151+i][3]))
    income=[]
    print profit
    num=0.0
    m=0

...
from logistic4 import LogisticRegression, load_data ,shared_dataset
...
def test_SdA(finetune_lr=0.1, pretraining_epochs=10,
    ...
    while testday<55:
        ...
        #while n<1:
            ...
            for k in xrange(sda.n_layers):
                # go through pretraining epochs
                for epoch in xrange(pretraining_epochs):
                    # go through the training set
                    c = []
                    for batch_index in xrange(n_train_batches):
                        c.append(pretraining_fns[k](index=batch_index,
                                 corruption=corruption_levels[k],
                                 lr=pretrain_lr))
                    print 'Pre-training layer %i, epoch %d, cost ' % (k, epoch),
                    print numpy.mean(c)
            ...
            while (epoch < training_epochs) and (not done_looping):
                ...
                for minibatch_index in xrange(n_train_batches):
                    ...
                    if patience <= iter:
                        done_looping = True
                        break
            for k1 in xrange(n_test_batches):

                predition=[pred_model(k1)]
                print predition
                pred_data.extend(predition)
            end_time = time.clock()
            print(

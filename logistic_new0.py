...
def load_data():
    ...
    try:
        ...
        changetime=['2012-07-02','2013-01-04','2013-07-01','2013-09-18','2013-12-16','2014-06-16','2014-12-15','2015-01-26','2015-05-21','2015-06-15']
        y_date='2015-06-19'
        #x='2014-06-17'
        stocklist=[]
        nun=0
        ...
        temp=cur.execute("select open,close from if888_m1 where date_time < %s and date_time > %s",[y_date,changetime[0]])
        #c='`000001.SZ`'
        #print c
        #cur.execute("select * from %s" %str.lower(c))
        m=cur.fetchall(temp)
    ...
    day4=len(stocklist[4][0])/242
    day5=len(stocklist[5][0])/242
    day6=len(stocklist[6][0])/242
    day7=len(stocklist[7][0])/242
    day8=len(stocklist[8][0])/242
    day=day0+day1+day2+day3+day4+day5+day6+day7+day8
    print day0,day1,day2,day3,day4,day5,day6,day7,day8,day
    ...
    for i in range(day4):
        b=[]
        for j in range(300):
            b.extend(stocklist[4][j][i*242:(i+1)*242])
        a.append(b)
    for i in range(day5):
        b=[]
        for j in range(300):
            b.extend(stocklist[5][j][i*242:(i+1)*242])
        a.append(b)
    for i in range(day6):
        b=[]
        for j in range(300):
            b.extend(stocklist[6][j][i*242:(i+1)*242])
        a.append(b)
    for i in range(day7):
        b=[]
        for j in range(300):
            b.extend(stocklist[7][j][i*242:(i+1)*242])
        a.append(b)
    for i in range(day8):
        b=[]
        for j in range(300):
            b.extend(stocklist[8][j][i*242:(i+1)*242])
        a.append(b)
    length=len(a)
    print len(a),'a'
    '''
    ...
    train_set_x=set_x[0:240]
    valid_set_x=set_x[0:240]
    test_set_x=set_x[0:240]
    train_set_y=y[6:246]
    valid_set_y=y[6:246]
    test_set_y=y[6:246]
    '''
    '''
    test_set_x=valid_set_x=train_set_x=a[0:600]
    test_set_y=valid_set_y=train_set_y=y[0:600]
    '''
    train_set_x=a[0:500]
    valid_set_x=a[600:]
    test_set_x=a[500:600]
    train_set_y=y[0:500]
    valid_set_y=y[600:]
    test_set_y=y[500:600]

    print test_set_y[0:10]
...
def sgd_optimization_mnist(learning_rate=0.13, n_epochs=1000,
    ...
    classifier = LogisticRegression(input=x, n_in=6*300*242, n_out=2)
    ...
    while (epoch < n_train_batches) and (not done_looping):
        ...
        #print classifier.W.get_value()
        # iteration number
        iter = (epoch - 1) * n_train_batches+epoch

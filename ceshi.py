...
#stocklist[0][0][0]表示的是股票每分钟的６个数据
nun=0
#print str.lower(list(i)[0])
#,high double,low double ,close double ,volume double ,amt double
for i in a:
    try:
        #print i
        print str.lower(list(i)[0])
        ...
        #b=cur.execute("select * from `%s`" %str.lower(list(i)[0]))
        b=cur.execute("select open,high,low,close,volume,amt from `%s`"%str.lower(list(i)[0]))
        #print b
        #data.append(cur.fetchall())
        if (b!=0):
